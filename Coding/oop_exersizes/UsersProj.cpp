...
void add_friend(vector<Users>& users, int user1_id, int user2_id) {
    for (int i = 0; i < users.size(); i++) {
        ...
        if (user.get_id() == user1_id || user.get_id() == user2_id) {
            ...
            if (user.get_friends().find(user2_id + " ") == string::npos) {
                new_friend_id = user.get_friends();
            } else {
                new_friend_id = user.get_friends().substr(0, user.get_friends().find(user2_id + " ")) +
                                 to_string(user1_id) + " " +
                                 user.get_friends().substr(user.get_friends().find(user2_id + " ") + 3);
            }
...
int main() {
    string filename; 
    cin>>filename; 
    vector<Users> users = initialization(filename);
